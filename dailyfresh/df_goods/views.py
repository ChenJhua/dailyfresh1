...
def list(request, tid, pindex, sort):
    goodstype = GoodsType.objects.get(pk=int(tid))
    news = goodstype.goodsinfo_set.order_by('-id')[0:2]
    if sort == '1':  # 默认， 最新
    ...
    context = {'title': goodstype.ttitle, 'guest_cart': 1,
               'page': page, 'paginator': paginator,
               'typeinfo': goodstype, 'sort': sort,
               'news': news, 'cart_count': cart_count(request)
               }
...
def detail(request, id):
    ...
    context = {'title': goods.gtype.ttitle, 'guest_cart': 1,
               'g': goods, 'news': news, 'id': id, 'cart_count': cart_count(request)
               }
